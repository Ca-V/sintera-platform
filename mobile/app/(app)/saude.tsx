...
// Saúde / Indicadores — consome GET /api/biomarkers/organized (mesmo objeto canônico
// da Web: Relatório, Dashboard e Timeline usam a MESMA organização). Sem juízo clínico:
// categorias + faixa aritmética. O ponto colorido é status de faixa (não diagnóstico).
...
export default function SaudeScreen() {
  ...
  const load = useCallback(async () => {
    setError(null)
    try {
      const data = await api.get<Organized>('/api/biomarkers/organized')
        ...
        ListEmptyComponent={
          loading ? <Loading /> : (
            <Text style={{ color: colors.mauve, fontSize: font.size.sm }}>
              {error ?? 'Envie um exame para ver seus indicadores.'}
            </Text>
          )
        }
