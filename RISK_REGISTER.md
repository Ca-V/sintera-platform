...
| R-010 | Arquitetura | **Ponte transitória de análise** — o Mobile reusa a rota `/analyze` da Web (ADR-020); acopla o Mobile à URL/deploy da Web | **Monitorado** | Aceito como TRANSITÓRIO (fundadora 2026-07-31) para não duplicar a regra de extração nem atrasar a Onda 1. **Retirada (backlog pós-Onda-1):** migrar a extração para uma **camada compartilhada** (Edge Function/serviço comum) consumida por Web+Mobile, eliminando o acoplamento. A camada de auth compartilhada (`getAuthedSupabase`) já prepara o terreno. [ADR-020](docs/adr/ADR-020_PONTE_ANALISE_MOBILE_TRANSITORIA.md). |
| R-009 | Segurança / Infraestrutura | **Hardening do bucket `exams`** — sem `allowed_mime_types` nem `file_size_limit` no servidor (validação de MIME/tamanho só no cliente). Defesa em profundidade; RLS já confina cada usuário à própria pasta; mesma postura da Web hoje | **Monitorado** | **Melhoria de infra COMPARTILHADA Web+Mobile — janela própria** (decisão fundadora 2026-07-31): executar em janela dedicada, após **avaliação de impacto** e **homologação cruzada** (Web e Mobile). Migration curta: `allowed_mime_types` (pdf/jpeg/png/heic) + `file_size_limit` (~20 MB) no bucket `exams`. **Não bloqueia** o Inc.6 (cliente valida; RLS por owner). Verificado por consulta read-only ([MOBILE-027 §8](docs/MOBILE-027_READINESS_INCREMENTO6_UPLOAD.md)). |
