...
export function ConditionsScreen() {
  ...
  function startNew() { setEditing(null); setScope('propria'); setName(''); setRelative(''); setSince(''); setNotes(''); setOpen(true) }
  function startEdit(c: ConditionDTO) {
    setEditing(c); setScope(c.scope); setName(c.name); setRelative(c.relative ?? ''); setSince(c.since_label ?? ''); setNotes(c.notes ?? ''); setOpen(true)
  }
  async function save() {
    if (!name.trim()) { Alert.alert('Nome obrigatório', 'Informe o nome da condição.'); return }
    setSaving(true)
    try {
      const input: ConditionInput = { id: editing?.id, scope, name, relative, since_label: since, notes }
