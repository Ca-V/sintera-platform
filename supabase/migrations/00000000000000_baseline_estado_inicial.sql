...
create policy "Usuários criam seus scores" on public.biological_scores for insert with check (auth.uid() = user_id);
 ...
 insert into storage.buckets (id, name, public)
  ...
  create table if not exists public.exams (
    id         uuid        default gen_random_uuid() primary key,
    user_id    uuid        references auth.users on delete cascade not
   null,
    type       text,
    exam_date  date,
    file_url   text,
  ...
  create policy "storage_exams_insert" on storage.objects
    for insert with check (
      bucket_id = 'exams'
      and auth.uid()::text = (storage.foldername(name))[1]
    );

  create policy "storage_exams_select" on storage.objects
    for select using (
      bucket_id = 'exams'
      and auth.uid()::text = (storage.foldername(name))[1]
    );

  create policy "storage_exams_delete" on storage.objects
    for delete using (
      bucket_id = 'exams'
      and auth.uid()::text = (storage.foldername(name))[1]
    );alter table public.biological_scores
  add column if not exists exam_id uuid references public.exams on delete cascade;
alter table public.ai_insights
  add column if not exists exam_id uuid references public.exams on delete cascade;
create index if not exists biological_scores_exam_id_idx on public.biological_scores (exam_id);
create index if not exists ai_insights_exam_id_idx on public.ai_insights (exam_id);
create index if not exists biomarkers_exam_id_idx on public.biomarkers (exam_id);
...
create index if not exists biological_scores_user_id_idx on public.biological_scores (user_id);
...
alter table public.exams add constraint exams_status_check
  check (status in ('pending', 'processing', 'processed', 'error'));-- Tabela de registros diários do usuário
create table public.daily_logs (
  id uuid default gen_random_uuid() primary key,
  user_id uuid references auth.users(id) on delete cascade not null,
  energy numeric(4,1),       -- 0–10
  sleep_hours numeric(4,1),  -- horas de sono
  water_liters numeric(4,2), -- litros de água
  mood integer,              -- 0–10
  logged_at timestamptz default now() not null
);

-- Índice para consultas por usuário/data
create index daily_logs_user_date_idx on public.daily_logs (user_id, logged_at desc);
