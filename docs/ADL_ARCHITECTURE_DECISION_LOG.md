...
| 2026-08-11 | **Modelo Clínico Canônico** (diretriz permanente): o núcleo da plataforma é um **modelo canônico de conhecimento clínico** (Observação como espinha + Condition + Intervention; concept-coded/FHIR-aligned; com proveniência e `effectiveTime`). Exames, wearables, Apple Health, Google Health Connect, Garmin, WHOOP, FHIR, laboratórios e demais integrações são **fontes** que convergem ao modelo comum. Pipeline, persistência, regras e consumidores dependem do **canônico**, não da implementação de cada fonte — preservando contexto, proveniência e comparabilidade. "Evento" é gatilho, não centro. Complementa CAP-002 (Capture Hub: ingestão → 1 pipeline) no nível do **modelo de domínio**; **alerta**: `wearable_readings` (FASE-2) separado de `biomarkers` perpetua modelos paralelos e deve convergir ao canônico. Evolução de domínio futura — não reescrever agora | Estrutural (modelo) | Alto | Aprovado (diretriz permanente) | CAP-002 §2 · `docs/INGESTAO-PIPELINE.md` §9–§10 |
