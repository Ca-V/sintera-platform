...
1. **BETA-2 · Captura única institucional (rollout completo)** `ARQ` — **ADR gate = CAP-001** (spec congelada e aprovada pela fundadora; não duplicar). O componente `CreateRecordMenu` JÁ é completo (arquivo/foto/manual + **voz** via slot + **arrastar** via drop→bundle + multipágina) — BETA-2 é puro **rollout de adoção**. **Adoção atual:** ✅ Exames · ✅ Medicamentos · ✅ Condições · ✅ **Medidas (17/07)** · ❌ Recursos · ❌ Contracepção(ciclo) · ❌ Agenda · ❌ Documentos(inexistente) · ❌ Sinais Vitais · ❌ Hábitos. Meta: `CreateRecordMenu` como ÚNICO "Adicionar" em cada um (os módulos com passo dedicado — Medidas/Contracepção/Recursos — recebem a captura ao chegar seu passo; os demais entram aqui). Nenhum `<input type=file>` avulso.
2. ☑ **BETA-7 · Modelo financeiro universal** — FIN-001 criado (Evento=portador, Despesas=projeção, sem tabela por domínio). Tipo de documento fiscal (NF/recibo/comprovante/outro): migration 119 aplicada + campo no AgendarModal (modo despesa) + exibição em Despesas. Núcleo puro `lib/finance/expense` testado. Fluxo único "registrar despesa" = AgendarModal modo expense (reúso; sem formulário por módulo). Rollout aos demais módulos entra nos seus passos (Contracepção/Recursos).
3. ☑ **BETA-3 · Exames com integração financeira** — o exame captura valor + documento fiscal (NF/recibo/comprovante) via AgendarModal modo despesa (FIN-001/BETA-7), vinculado ao exame (EVT-C6), alimentando Despesas (projeção)/Histórico/Relatórios/Agenda. Fecha o loop: o detalhe do exame exibe a **despesa vinculada** (valor pago + link do documento → Despesas). Reúso total do fluxo financeiro único.
4. **BETA-WEAR · Infraestrutura de Wearables (Connector Layer / HIP-001)** `ARQ` · ▣ EM ANDAMENTO — **ciclo 1 ☑:** WEA-001 criado (ARCH-000 §4) + registrado; núcleo puro `lib/connectors` (ConnectorDescriptor·CanonicalSample com proveniência·SyncRun·reconcileSamples/dedupWithinSource — reconciliação/dedup multi-provedor preservando origem; +6 testes); migration 120 aplicada (`connector_sync_runs` histórico + `external_id`/`connector_version` de proveniência em `wearable_readings`). **Próximos ciclos:** registro de conectores · Strava OAuth+sync (até credenciais) · orquestrador auto+manual · propagação Timeline/Indicadores/Registros · Painel Operacional · Garmin até auth+sync. Concluir a Connector Layer prevista no HIP-001: conector vendor-neutral (Adapter), OAuth2 quando aplicável, sync automática+manual, proveniência da fonte, persistência no Modelo Canônico (UCDA), propagação p/ Timeline·Indicadores·Registros de Saúde, novos conectores sem alterar domínios; dados mínimos (atividades·passos·distância·calorias·FC·sono·carga/recuperação·VO₂máx quando disponíveis), **nunca substituindo a fonte** (proveniência preservada). Conectores P1: Garmin·Strava·WHOOP; P2: Apple Health·Health Connect. **VER RESTRIÇÕES DURAS ABAIXO** (credenciais de provedor = dep. externa; Apple Health/Health Connect = exigem app NATIVO, não conectam via web). Alcance autônomo agora: **toda a infraestrutura + 1 conector de referência em código até a fronteira de credenciais**.
5. ☑ **BETA-5 · Medidas** — CONCLUÍDO (adiantado junto da captura, um só toque): os dois botões ("Adicionar" + "Escanear bioimpedância") viraram **um** `CreateRecordMenu` ("Adicionar medida": foto/arquivo/manual); bioimpedância **auto-detectada** no processamento (`onScanFile`), sem botão dedicado; form ganhou "Cancelar". Também marca Medidas ✅ na adoção da captura (BETA-2). *(PDF de laudo de bioimpedância = follow-up: o endpoint de visão é só-imagem.)*
