...
| **FB-004** | Recursos de Saúde | Unificar os **dois botões** num só fluxo institucional; restaurar **Tipo→Outros**; faltam **valor · doc fiscal · anexos · recorrência**. | 🟡 parcial 17/07 | **Parte A ✅:** os 2 botões ("Escanear"+"Adicionar") viraram um `CreateRecordMenu` (foto/arquivo/manual; foto de receita→grau auto); tipo **"Outros"** restaurado (migration 122 amplia a constraint + entrada no seletor); form ganhou Cancelar. **Parte B (pendente):** valor pago + tipo de doc fiscal + anexo fiscal + recorrência (reusar FIN-001; ex. recorrência = troca de lentes) — próximo ciclo. |
