...
| NC-0012 | 15/07 | Claude | Auditoria estática | Eventos | EVT-F001 | Funcional | média | 🟠 aberta | `professional_kind` é **lido e renderizado** em 3 telas (`timeline:167,289` · `relatorio:219,235` · `r/[token]:87,243` via `PROF_LABEL`), mas **nenhum caminho de escrita** o preenche: o modal não tem campo de tipo de profissional e `healthEventToRow` repassa sempre `null`. Feature de leitura sem produtor |
| NC-0013 | 15/07 | Claude | Auditoria estática | Eventos | EVT-F008 | Dados | média | 🟠 aberta | Eventos legados que vivem só em `agenda_events` aparecem na **Agenda** (usa o repositório com dedup) mas **nunca** no Histórico/Relatório/relatório compartilhado, que consultam `health_events` **direto** (`timeline:135` · `relatorio:219` · `r/[token]:87`), ignorando `agenda_events`. Leitura divergente do mesmo domínio por tela |
...
| NC-0015 | 15/07 | Claude | Auditoria estática | Eventos | EVT-F004 | Funcional | média | 🟠 aberta | Worker de lembretes (`api/agenda/reminders`) é completo, mas o **cron não está versionado em migração** (só passo manual em `docs/agenda-lembretes-setup.md`), ao contrário dos alertas de pipeline (cron em migração 016). `reminder_enabled` é capturado mas o disparo diário não é reprodutível/garantido — em ambiente sem o passo manual, lembretes nunca disparam. Agendar em produção depende de URL/segredo (impedimento parcial) |
...
| NC-0019 | 15/07 | Claude | Auditoria estática | Eventos | EVT-F003 | Dados | baixa | 🟠 aberta | `service.ts:94` carimba `source:'recurrence'` nas ocorrências roladas, mas `'recurrence'` **não está no union** `EventSource` (`event.ts:79-81`) → divergência tipo↔dado; `isDerived()` classifica toda ocorrência recorrente como derivada (sem consequência de UI hoje) |
