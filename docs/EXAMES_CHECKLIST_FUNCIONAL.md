...
| NC-0009 | 15/07 | Claude | Auditoria estática | Exames | EXA (CPE) | Funcional | média | 🟢 corrigida 17/07 (EXA-C3) | `clinical_results` (parâmetros não-biomarcador via CPE, ex. Pentacam) é persistido; `clinicalResultsToUcda` existia mas nenhum fluxo o lia → dado clínico invisível. **Corrigido:** read-side GENÉRICO via UCDA (`groupUcdaForDisplay` puro + `ClinicalResultsCard` + carga em `exams/[id]`), respeitando Convergência Progressiva (sem lógica por modalidade). Presentação rica por modalidade (E6) segue futura; homologação com Pentacam real = EXA-D1 (dep. de documento real) |
...
| NC-0028 | 15/07 | Claude | Auditoria estática | Exames | EXA-F010 (processador corneal) | Dados | baixa | ✅ encerrada | commit `49b58a3` · `eyeOf` (lateralidade no processador Pentacam) usava `/\bO[SE]\b/i` case-insensitive → o **artigo português "os"** casava como olho esquerdo (OE) → parâmetro numa linha com "os" (sem OD) seria atribuído ao olho ERRADO. Fix: abreviações OD/OS/OE (oculus dexter/sinister) casam case-SENSITIVE (maiúsculas); "olho direito/esquerdo" segue case-insensitive. `FUNC-corneal-tomography-processor` (+caso "os"). Encerrada no mesmo ciclo |
| NC-0027 | 15/07 | Claude | Auditoria estática | Exames | EXA-F010 (Identidade Clínica) | Dados | baixa | ✅ encerrada | commit `b8f6f5d` · `identifyClinical`: `/\bCC\b/` (crânio-caudal) era evidência FORTE (peso 3) de Mamografia — genérico demais → um documento com "CC" isolado atingia `MIN_SCORE` e era misclassificado como Mamografia. Fix: `CC` rebaixado a moderado (peso 1). Mamografia real sempre traz mamografia/BI-RADS/MLO (fortes) → recall preservado (todos os testes verdes). `FUNC-clinical-identity-registry` (+caso CC isolado). Encerrada no mesmo ciclo |
| NC-0026 | 15/07 | Claude | Auditoria estática | Exames | EXA-F004 (Identity Validator) | Dados | baixa | ✅ encerrada | commit `c29cd02` · `baseTitle` (heurística de super-segmentação) tinha `LATERALITY_RE` com bare `\bd\b`/`\be\b` → "VITAMINA/HEPATITE D" e "…E" viravam o mesmo baseTitle → 2 exames DISTINTOS adjacentes (mesma data/emissor) eram falsamente marcados `needs_review` técnico (BLOQUEADOS). Fix: remove bare D/E do regex (laterialidade real = OD/OE/direito/esquerdo/bilateral, mantidos). Reduz estritamente falsos-positivos. `FUNC-identity-validator` (+caso Hepatite D×E). Encerrada no mesmo ciclo |
...
- **`processBundle` + `structural-analysis`** (F007/F010): ✅ auditado, sem alteração — orquestrador delega correto; detecção por página com divisão-por-zero **guardada** (`letters.length<3`), `detectKind` com precedência `results`, `countMatches` correto. Coberto por `FUNC-segmentation`/`FUNC-structural-analysis`.
