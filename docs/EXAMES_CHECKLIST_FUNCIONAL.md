...
| NC-0021 | 15/07 | Claude | Auditoria estática | Exames | EXA-F002 (nomenclatura) | Dados | média | ✅ encerrada | commit `<pendente>` · `deriveDisplayTitle` colapsava TODA urina em "Urina tipo I" (`isUrine` casa urocultura/urina-24h/sedimento/EAS) → uma **urocultura** era renomeada "Urina tipo I", perdendo a Identidade Documental (fere RDC 657 "transcreve, não infere"). Fix: só urina de ROTINA vira "Urina tipo I"; urocultura e urina-24h preservam o nome fiel. Casos no `ARCH-002`. Encerrada no mesmo ciclo |
