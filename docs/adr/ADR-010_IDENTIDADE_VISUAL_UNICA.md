...
4. **Direção cromática oficial = A·E** ([[COLOR-001]]) — inspirada na **atmosfera** de _Almond Blossom_ (não reprodução):
   traduz serenidade, continuidade, sofisticação, natureza e leveza. **Tom-âncora = `#579DA8`** (azulado suave, ~188°, 7ª
   etapa da rampa; validado em Mobile) — é o **piso escuro** dos preenchimentos (botões/CTA, item ativo, nós, gráficos),
   com **texto branco**; nada institucional mais escuro que ele (o tom `#4D8C9D` fica só em texto/detalhe). Os **tints
   claros puxam para o VERDE** (~165–180°), compondo uma rampa com **gradiente de matiz verde (claros) → azul (âncora)**.
   Detalhes em [[COLOR-001]]. **Congela-se a direção**; tokens 50–900 (com pequenos ajustes técnicos de contraste/acessibilidade/dark)
