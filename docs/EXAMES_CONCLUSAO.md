...
| E6 | **Fluxo único de upload** (PDF/foto/scan/imagem única/múltiplas/multipágina) | C1 / §2.5 | Bundle→CDU (existe) · `useDocumentBundle` | um só ponto de entrada; sem dropzone duplicada; usa Bundle→CDU | 🔄 **parte 1 feita** (exames adota `useDocumentBundle`+`DocumentBundleStaging` — ganha reordenar/Galeria, remove staging duplicado). **Parte 2 = rotear ômicas pelo fluxo único** (remoção do fork) — **decisão de produto pendente** (mecânica de entrada de ômicas) |
