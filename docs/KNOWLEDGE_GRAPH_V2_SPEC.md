...
**Status:** Sprint 1 do KG v2 — **especificação revisável**. **Documento apenas: sem código, sem banco, sem migrations, sem importação de fontes.** Descreve *o que* o KG relaciona e *o que ele nunca faz*; a modelagem física e a implementação são etapas posteriores (gated por Design Review).
...
- **Relationship** — a aresta científica entre entidades (é entidade de primeira classe: carrega evidência, fonte, versão, força).
