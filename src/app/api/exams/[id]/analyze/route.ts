...
export async function POST(request, { params }) {
  ...
  await supabase.from('exams').update({ status: 'processing' }).eq('id', examId)
  await supabase.from('biomarkers').delete().eq('exam_id', examId)
  const bmRows = parsed.biomarkers.map(b => ({
  ...
  const { error: bmErr } = await supabase.from('biomarkers').insert(bmRows)
  if (bmErr) {
    await supabase.from('exams').update({ status: 'error' }).eq('id', examId)
    return NextResponse.json({ error: bmErr.message }, { status: 500 })
  }
  const bm = parsed.biomarkers.map(b => ({ ...b, category: 'metabolic' }))
  const scores = calcScores(bm)
  await supabase.from('biological_scores').insert({ exam_id: examId, user_id: user.id, ...scores })
  const insights = buildInsights(bm, user.id)
  await supabase.from('ai_insights').insert(insights.map(i => ({ ...i, exam_id: examId })))
  await supabase.from('exams').update({ status: 'processed', type: parsed.exam_type }).eq('id', examId)
