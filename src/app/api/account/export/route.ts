...
export async function GET() {
  try {
    ...
    const [
      ...
      (supabase as any).from('biomarkers').select('name,value,unit,reference_min,reference_max,status,exam_id,collected_at,created_at').eq('user_id', userId),
      supabase.from('ai_insights').select('insight,category,priority,is_read,created_at').eq('user_id', userId),
    ])
