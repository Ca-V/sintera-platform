...
export async function POST(req: NextRequest) {
  ...
  // Lembretes vencendo hoje/amanhã (coexistência canônico+legado, dedup) — pelo DOMÍNIO dono.
  // O cron é só mais um consumidor (ator de sistema) das capacidades da Agenda; não conhece
  // as tabelas de evento nem reimplementa a dedup.
  const { query, command } = eventServicesFor(admin)
  let items
  try {
    items = await query.listDueReminders(today, tomorrow)
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : 'Falha ao listar lembretes' }, { status: 500 })
  }
  if (items.length === 0) {
