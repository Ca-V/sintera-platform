  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="font-body text-sm text-mauve">Gerencie seus dados pessoais e preferências</p>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <button className="ml-auto p-2 text-mauve hover:text-petal transition-colors">
          <Edit3 size={16} />
        </button>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        <h2 className="font-body text-sm font-semibold text-onyx/60 uppercase tracking-widest">Dados pessoais</h2>

        <div className="grid sm:grid-cols-2 gap-4">
          {[
            { label: 'Nome',          value: profile?.name ?? '—' },
            { label: 'Faixa etária',  value: profile?.age_range ?? '—' },
            { label: 'Ciclo médio',   value: profile?.cycle_length ? `${profile.cycle_length} dias` : '28 dias' },
      ...
      {(profile?.goals?.length ?? 0) > 0 && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <h2 className="font-body text-sm font-semibold text-onyx/60 uppercase tracking-widest mb-4">Meus objetivos</h2>
          <div className="flex flex-wrap gap-2">
            {(profile?.goals ?? []).map(id => {
              const g = goals.find(x => x.id === id)
              if (!g) return null
              return (
                <span key={id} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-body font-medium bg-blush text-petal-dark border border-petal-light">
                  {g.emoji} {g.label}
                </span>
              )
            })}
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        className="card-premium p-6">
        <h2 className="font-body text-sm font-semibold text-onyx/60 uppercase tracking-widest mb-4">Notificações</h2>
        <div className="flex flex-col gap-3">
          {[
            ...
            <div key={p.label} className="flex items-center justify-between py-2 border-b border-border last:border-0">
              ...
              <span className={`text-xs font-body font-medium px-2.5 py-1 rounded-full ${p.active ? 'bg-sage-light text-sage' : 'bg-ivory text-mauve'}`}>
      ...
      <button onClick={save}
        className="w-full flex items-center justify-center gap-2 gradient-sintera text-white font-body font-medium py-3.5 rounded-full hover:opacity-90 active:scale-[0.98] transition-all shadow-md">
