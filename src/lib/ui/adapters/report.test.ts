...
// Os adapters consomem HealthEvent[] (domínio); aqui montamos o domínio a partir da linha.
const ev = (o: Partial<HealthEventRow>) => rowToHealthEvent({ id: 'id', event_type: 'consulta', title: 'Consulta', event_date: '2026-06-10', ...o })

const input = (o: Partial<ReportInput> = {}): ReportInput => ({
  name: 'Carina', objective: 'consulta', bioRows: [], events: [], examRows: [], generatedAt: '2026-07-01', ...o,
})
...
describe('reportAdapter — determinismo (seções e ordem estáveis)', () => {
  ...
  it('buildReport: mesma entrada → mesma saída', () => {
    const i = input({ examRows: exams, events: [ev({ id: 'x' }), ev({ id: 'y', event_date: '2026-06-12' })] })
...
describe('reportAdapter — Estado 2 sai vazio, leitura sai real', () => {
  it('situação (catálogo) vazia; resumo conta exames/eventos reais', () => {
    const model = buildReport(input({ examRows: [exam({}), exam({ id: 'e2' })], events: [ev({})] }))
    expect(model.situation).toEqual([])
    const summary = Object.fromEntries(buildSummary([exam({}), exam({ id: 'e2' })], [ev({})]).map((s) => [s.label, s.value]))
  ...
  it('cover: período derivado das datas reais', () => {
    const model = buildReport(input({ examRows: [exam({ exam_date: '2026-03-01' })], events: [ev({ event_date: '2026-07-01' })] }))
...
describe('reportAdapter — timeline reusa o adapter validado', () => {
  it('linha do tempo limita a 6 e mantém o formato do TimelineEvent', () => {
    const rows = Array.from({ length: 8 }, (_, i) => ev({ id: 'e' + i, event_date: `2026-06-0${(i % 9) + 1}` }))
