  ...
  return (
    <Link href={href} onClick={onClose} {...hintProps}
      className={cn(
        ...
        active
          ? 'nav-active-glow bg-white/20 text-[#F5EFE4]'
          : 'text-[#F5EFE4]/85 hover:text-[#F5EFE4] hover:bg-white/10'
      )}
    >
      <Icon size={16} className={cn('flex-shrink-0 transition-colors',
        active ? 'text-[#F5EFE4]' : 'text-[#F5EFE4]/65 group-hover:text-[#F5EFE4]')} />
      <span className={cn('flex-1', active && 'font-medium')}>{label}</span>
      {soon && (
        <span className="font-body text-[9px] font-medium text-[#F5EFE4]/80 bg-white/12 px-1.5 py-0.5 rounded-full border border-white/15">
          Em breve
        </span>
      )}
    </Link>
...
function SidebarContent({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full select-none" style={{ background: 'radial-gradient(ellipse 90% 40% at 50% 0%, rgba(255,255,255,0.10) 0%, transparent 70%), linear-gradient(to bottom, #4C8A9C 0%, #396F82 100%)' }}>
      ...
      <div className="flex items-center justify-between px-5 py-4">
        <Link href="/dashboard" onClick={onClose} className="flex items-center gap-2.5">
          ...
          <span className="font-display text-lg font-semibold tracking-[0.2em] text-[#F5EFE4]">SINTERA</span>
        </Link>
        <button onClick={onClose} className="lg:hidden text-[#F5EFE4]/70 hover:text-[#F5EFE4] transition-colors">
          <X size={17}/>
        </button>
      </div>
      ...
      <Link href="/dashboard/profile" onClick={onClose}
        className="mx-4 mb-3 p-3 rounded-2xl border border-white/15 bg-white/8 hover:bg-white/12 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-body font-medium text-[#F5EFE4] leading-tight truncate">{displayName}</p>
          </div>
          <ChevronRight size={13} className="text-[#F5EFE4]/60 flex-shrink-0"/>
        </div>
      </Link>
