...
export function eventToInput(ev: HealthEvent): Partial<AgendaEventInput> {
  ...
  return {
    ...
    modality: ev.modality ?? '', professionalKind: ev.professionalKind ?? '', professionalName: ev.professionalName ?? '',
...
export function useEventForm() {
  ...
  async function saveEvent(userId: string, input: AgendaEventInput, editing?: HealthEvent | null, links?: EventLink[]): Promise<void> {
    ...
    await services.command.create(userId, {
      ...
      professionalKind: input.professionalKind || null,
      ...
      expenseDocType: input.expenseDocType || null,
