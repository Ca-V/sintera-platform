...
const subMetrics = [
  ...
  { label: 'Sono',        value: '7.5h', icon: BedDouble, color: '#579DA8' },
  { label: 'Ciclo',       value: 'Dia 8',icon: Moon,     color: '#B15C4C' },
]

function label(score: number) {
  if (score >= 85) return { text: 'Excelente', color: '#7E9B6E' }
  if (score >= 70) return { text: 'Boa',       color: '#C9A97A' }
  if (score >= 50) return { text: 'Regular',   color: '#579DA8' }
  return { text: 'Baixa', color: '#B15C4C' }
}
  ...
  return (
    ...
    >
      ...
      <div className="absolute -bottom-12 -left-12 w-40 h-40 rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(151, 201, 195,0.08) 0%, transparent 60%)' }}/>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative w-32 h-32 flex-shrink-0">
          ...
          <div className="absolute inset-0 rounded-full blur-xl opacity-20"
            style={{ background: `conic-gradient(#B15C4C, #579DA8, transparent)` }}/>
          <svg viewBox="0 0 128 128" className="w-full h-full -rotate-90 relative">
            ...
            <defs>
              <linearGradient id="scoreGrad" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor="#B15C4C"/>
                <stop offset="100%" stopColor="#579DA8"/>
              </linearGradient>
            </defs>
          </svg>
