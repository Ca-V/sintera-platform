...
const subMetrics = [
  ...
  { label: 'Sono',        value: '7.5h', icon: BedDouble, color: '#A89CBD' },
  { label: 'Ciclo',       value: 'Dia 8',icon: Moon,     color: '#C2849A' },
]

function label(score: number) {
  if (score >= 85) return { text: 'Excelente', color: '#7DAF9E' }
  if (score >= 70) return { text: 'Boa',       color: '#C9A97A' }
  if (score >= 50) return { text: 'Regular',   color: '#A89CBD' }
  return { text: 'Baixa', color: '#C2849A' }
}
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-16 -right-16 w-56 h-56 rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(194,132,154,0.12) 0%, transparent 65%)' }}/>
      <div className="absolute -bottom-12 -left-12 w-40 h-40 rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(168,156,189,0.08) 0%, transparent 60%)' }}/>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative w-32 h-32 flex-shrink-0">
          ...
          <div className="absolute inset-0 rounded-full blur-xl opacity-20"
            style={{ background: `conic-gradient(#C2849A, #A89CBD, transparent)` }}/>
          <svg viewBox="0 0 128 128" className="w-full h-full -rotate-90 relative">
            ...
            <defs>
              <linearGradient id="scoreGrad" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor="#C2849A"/>
                <stop offset="100%" stopColor="#A89CBD"/>
              </linearGradient>
            </defs>
          </svg>
